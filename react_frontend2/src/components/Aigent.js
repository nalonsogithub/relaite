...
//  }, [messages]);
  ...
  // Effect to scroll chat log to top and check if scroll button should be shown
  useEffect(() => {
    if (chatLogRef.current) {
      chatLogRef.current.scrollTop = 0; // Scroll to top
      const isOverflowing = chatLogRef.current.scrollHeight > chatLogRef.current.clientHeight;
      setShowScrollButton(isOverflowing); // Show button only if there's overflow
    }
  }, [messages]);

  // Handle scrolling to the bottom with animation
...
//            console.log("Received chunk:", chunk);
			...
			// Introduce a delay to control the speed of chunk updates
			console.log('waiting');
			await new Promise(resolve => setTimeout(resolve, Math.random() * (300 - 100) + 0));  // Random delay between 100ms and 300ms
...
//  }
  ...
  .slick-slide {
    box-sizing: border-box;
    display: inline-block;  // Ensure slides are inline-block for proper layout
    width: 300px;  // Ensure each slide has a fixed width
	padding: 0 10px;
  }
...
useEffect(() => {
  if (sliderRef.current) {
    console.log('Slider dimensions:', sliderRef.current.innerSlider.list.offsetWidth);
  }
}, []);
