...
function ManageListing({buttonType}) {
  ...
  const columnsConversation = useMemo(() => [
    ...
    },
    {
        ...
        Cell: ({ row }) => (
            <>
                <input type="file" onChange={(e) => handleConversationFileChange(row.index, e.target.files[0])} />
                {row.original.imagePreviewUrl && <img src={row.original.imagePreviewUrl} alt="Preview"   style={{ width: 100 }} />}
            </>
        )
    },
    {
  ...
  const columnsDocument = useMemo(() => [
    ...
    },
    {
        ...
        Cell: ({ row }) => (
            <>
                <input type="file" onChange={(e) => handleDocumentFileChange(row.index, e.target.files[0])} />
            </>
        )
    },
    {
...
//    }));
	...
	console.log('flowData TO SEND', flowData)
      ...
      <form onSubmit={handleSubmit}>
	  	...
	  	<div className={styles.sectionBoxStyle2}>
			...
			<div className={styles.imageContainer}>
			  ...
			  />
			  {listingData.imagePreviewUrl && <ImagePreview src={listingData.imagePreviewUrl} alt="Listing Preview" />}
			</div>
		</div>
		...
		<div style={spacer}></div>
	  	...
	  	<div  className={styles.sectionBoxStyle2}>
			...
			<div className={styles.imageContainer}>
			  ...
			  />
			  {agentData.imagePreviewUrl_1 && <ImagePreview src={agentData.imagePreviewUrl_1} alt="Logo 1 Preview" />}
			</div>
			...
			<div className={styles.imageContainer}>
			  ...
			  />
			  {agentData.imagePreviewUrl_2 && <ImagePreview src={agentData.imagePreviewUrl_2} alt="Logo 2 Preview" />}
			</div>
						  ...
						  {cell.column.id === 'delete' ? (
							...
							<input
							  ...
							  onChange={e => {
								...
								console.log('Conversation:', file); // Or process the file
								handleConversationFileChange(cell.row.index, file); // Assuming passing index and file to the function 
							  }}
							/>
						  ) : (
							<input
						...
						<td {...cell.getCellProps()}>
						  {cell.column.id === 'delete' ? (
							...
							<input
							  ...
							  onChange={e => {
								const file = e.target.files[0]; // Correct way to get the file from input
								console.log('Document:', file); // Or process the file
