...
//  const [selectedCostType, setSelectedCostType] = useState(Array(data.length).fill(0));
  ...
  const { includedEstimates, setIncludedEstimates, 
		 selectedCostType, setSelectedCostType,
		customRenovation, setCustomRenovation} = useMortgage();	
	
  const [showChart, setShowChart] = useState(false);
//  const [showCustomRenovation, setShowCustomRenovation] = useState(false);
const [showCustomRenovation, setShowCustomRenovation] = useState(() => 
  !!(customRenovation.name || customRenovation.description || customRenovation.estimate)
);
...
useEffect(() => {
  if (customRenovation.name || customRenovation.description || customRenovation.estimate) {
    setShowCustomRenovation(true);
  }
}, [customRenovation]);
...
useEffect(() => {
  const totalCost = includedEstimates.reduce((sum, index) => {
    const cost = data[index].reno_cost_estimates[selectedCostType[index]];
    console.log(`Cost for ${data[index].room}: ${cost}`);
    return sum + cost;
  }, 0);

  const customCost = customRenovation.estimate ? parseFloat(customRenovation.estimate) : 0;
  setRenovationCosts(totalCost + customCost);

  console.log(`Total Cost: ${totalCost}, Custom Renovation Cost: ${customCost}`);
  ...
  const handleCostTypeChange = (event, index) => {
	...
	newSelectedCostType[index] = event.target.value;
          ...
          {data.map((item, index) => (
            <tr key={index}>
              ...
              <td>
                <Form.Control
                  as="select"
                  value={selectedCostType[index]} // Set the value based on selectedCostType
                  onChange={(e) => handleCostTypeChange(e, index)}
                  className={styles.tableField}
                >
				{item.reno_cost_list.map((cost, i) => (
				  <option key={i} value={i}>{cost}</option> // Ensure the option value is the index
				))}					  
                </Form.Control>
              </td>
              <td><Form.Control type="text" value={formatCurrency(item.reno_cost_estimates[selectedCostType[index]])} className={styles.tableField} readOnly /></td>
              <td>
                <Form.Check 
                  type="checkbox" 
